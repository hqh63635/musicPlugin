  ...
  if (newSong) {
    try {
      ...
      if (lyricResult && typeof lyricResult.lyric === 'string' && lyricResult.lyric.trim() !== '') {
        ...
        // 如果没有获取到有效歌词，生成模拟歌词
        const mockLyric = `[00:00.00]${newSong.title}[00:10.00]演唱：${newSong.singerName || newSong.artist}[00:20.00]暂无歌词[00:30.00]请欣赏音乐`;
  ...
  if (musicStore.currentSong) {
    try {
      ...
      if (lyricResult && typeof lyricResult.lyric === 'string' && lyricResult.lyric.trim() !== '') {
        ...
        // 如果没有获取到有效歌词，生成模拟歌词
        const mockLyric = `[00:00.00]${musicStore.currentSong.title}[00:10.00]演唱：${musicStore.currentSong.singerName || musicStore.currentSong.artist}[00:20.00]暂无歌词[00:30.00]请欣赏音乐`;
        musicStore.setLyric(mockLyric);
      }
    } catch (error) {
      console.error('获取歌词失败:', error);
